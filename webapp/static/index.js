...
function InfiniteScroller(displayHtmlId, numPokemonEachQuery, numPokemonPerRow) {
	...
	this.onReady = function() {
	    ...
	    loadPokemonCards(displayHtmlId, typeSelected, abilitySelected);
	...
	this.loadPokemonCards = function(displayHtmlId, typeFilter, abilityFilter) {
	    ...
	    .then(function(pokemonList) {
	        ...
	        for (var i = 0; i < pokemonList.length; i ++){
	            ...
	            if (i == this.numPokemonPerRow - 1) pokemonDisplayDiv += '</div>\n<div class="row">';
	        }
	        pokemonDisplayDiv += '</div>\n</div>';
	        this.curNumPokemonOnPage += numPokemonEachQuery;
	        ...
	        this.morePokemon = !(noResultsSatisfyCriteria || endOfQueryReached);
